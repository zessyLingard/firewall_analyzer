#!/usr/bin/env python3
"""
firewalld_audit.py — 10-point security audit for firewalld rulesets.

Usage:
    python firewalld_audit.py <file> [--admin-cidr CIDR [CIDR ...]]

Supports XML zone files, direct.xml, and CLI --list-all output.

Checks (identical to iptables_audit.py / nftables_audit.py):

  1.   Default INPUT/OUPUT policy — PASS if DROP/REJECT or explicit DROP/REJECT.
...
from firewalld_parser import load_rules


def main() -> None:
    ap = argparse.ArgumentParser(
        description="Audit firewalld ruleset — 10-point security checklist."
    )
    ap.add_argument("file", help="firewalld zone / direct.xml / CLI output file")
    ap.add_argument(
        "--admin-cidr",
        dest="admin_cidr",
        nargs="+",
    ...
    # Exit with non-zero on hard FAILs (not WARN)
    has_fail = (
        not policy_ok
